...
export default async function VotingOverviewPage() {
  const now = new Date();
  const events = await prisma.event.findMany({ where: { active: true, public: true, resultMethod: { in: ["PUBLIC_VOTE_ONLY", "JUDGE_AND_PUBLIC_VOTE"] }, votingOpenAt: { lte: now }, OR: [{ votingCloseAt: null }, { votingCloseAt: { gt: now } }], resultsPublishedAt: null, voteCandidates: { some: { active: true, public: true } } }, orderBy: { startsAt: "asc" }, select: { id: true, title: true, description: true, votingCloseAt: true } });
