...
export async function saveAlbumAction(data: FormData) {
  ...
  const payload = { title: text(data, "title"), description: text(data, "description") || null, eventId: text(data, "eventId") || null, coverImageUrl: cover, active: data.get("active") === "on", public: data.get("public") === "on", sortOrder: Number.parseInt(text(data, "sortOrder"), 10) || 0 };
...
export async function saveGalleryItemAction(data: FormData) {
  ...
  try { const payload = { title: text(data, "title") || "Uden titel", description: text(data, "description") || null, mediaType, imageUrl, videoUrl, thumbnailUrl, albumId, eventId, photographer: text(data, "photographer") || null, active: data.get("active") === "on", public: data.get("public") === "on", sortOrder: Number.parseInt(text(data, "sortOrder"), 10) || 0, createdById: actor.id }; if (id) await prisma.galleryImage.update({ where: { id }, data: payload }); else await prisma.galleryImage.create({ data: payload }); }
