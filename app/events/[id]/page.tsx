...
import EventDetailLocalFallback from "@/components/events/EventDetailLocalFallback";
import { getPublicEventRoute, getPublicEventRoutes, type PublicEventStatus } from "@/components/events/event-route-adapter";

export function generateStaticParams() {
  return getPublicEventRoutes().map((event) => ({ id: event.id }));
}

function statusClasses(status: PublicEventStatus) {
  const label = String(status);
  if (label.includes("ben")) return "border-green-400/30 bg-green-400/10 text-green-200";
  if (label.includes("pladser")) return "border-orange-400/30 bg-orange-400/10 text-orange-200";
...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const event = getPublicEventRoute(id);

  if (!event) {
    return (
      <main className="min-h-screen bg-black text-white">
        <Navbar />
        <EventDetailLocalFallback id={id} />
        <Footer />
      </main>
    );
  }
