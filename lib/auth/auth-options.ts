...
import { isActiveProfile, isAppRole, type AppRole, type AuthUser } from "./types";
...
function toAuthUser(user: {
  ...
  return {
    ...
    active: user.active,
    profileStatus: user.profileStatus === "INACTIVE" || user.profileStatus === "ARCHIVED" ? user.profileStatus : "ACTIVE",
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.profileStatus = authUser.profileStatus;
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.profileStatus = token.profileStatus === "INACTIVE" || token.profileStatus === "ARCHIVED" ? token.profileStatus : "ACTIVE";
