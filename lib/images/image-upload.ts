...
export type ImageUploadScope = "event" | "profile" | "sponsor" | "vehicle" | "gallery" | "team" | "vote";
...
export function imageBlobPath(scope: ImageUploadScope, ownerId: string, filename: string, uniqueId: string) {
  const folder = scope === "profile" ? "profiles" : scope === "gallery" ? "gallery" : scope === "team" ? "team" : scope === "vote" ? "votes" : `${scope}s`;
